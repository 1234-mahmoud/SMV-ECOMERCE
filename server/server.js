...
app.delete("/admin/categories/:id", authenticateToken, async (req, res) => {
  try {
    ...
    const productsWithCategory = await ProductModel.countDocuments({ category: categoryId });
...
app.put("/seller/products/:id", authenticateToken, upload.single("image"), async (req, res) => {
  try {
...
app.delete("/seller/products/:id", authenticateToken, async (req, res) => {
  try {
    if (req.user.role !== "Seller") {
      return res.status(403).json({ error: "Seller only" });
    }
...
app.put("/admin/products/:id", authenticateToken, upload.single("image"), async (req, res) => {
  try {
    if (req.user.role !== "Admin") {
      return res.status(403).json({ error: "Admin only" });
    }
...
// Admin: Update Seller
app.put("/admin/sellers/:id", authenticateToken, async (req, res) => {
  try {
    if (req.user.role !== "Admin") {
      return res.status(403).json({ error: "Admin only" });
    }
    ...
    if (email) {
      ...
      const existingUser = await UserModel.findOne({ email, _id: { $ne: sellerId } });
      if (existingUser) {
        return res.status(400).json({ error: "Email already registered" });
      }
      seller.email = email;
    ...
    await seller.save();
    res.status(200).json({ message: "Seller updated successfully", seller: { _id: seller._id, name: seller.name, email: seller.email, role: seller.role } });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
...
// Admin: Delete Seller
app.delete("/admin/sellers/:id", authenticateToken, async (req, res) => {
  try {
    if (req.user.role !== "Admin") {
      return res.status(403).json({ error: "Admin only" });
    }

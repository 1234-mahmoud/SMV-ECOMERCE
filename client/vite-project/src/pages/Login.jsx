...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (loginUser.fulfilled.match(result)) {
      const userRole = result.payload.user.role; // اجلب الدور من الـ payload
  ...
  return (
    <div className="flex justify-center items-center gap-10 h-lvh">
      <div className="welcome h-full hidden md:flex flex-col justify-center items-center gap-5 flex-1/2 text-[rgb(var(--main-color))] bg-[linear-gradient(135deg,#667eea_0%,#764ba2_100%)]">
        <h1 className="font-semibold text-4xl lg:text-5xl">Welcome to SMV-ECOM</h1>
      ...
      <div className="login_form flex-1/2 px-12">
        <form onSubmit={handleSubmit} className="flex flex-col gap-6">
          ...
          <button
            ...
            className={`text-[rgb(var(--main-color))] bg-blue-600 rounded-md py-3 text-xl my-3 ${status === "loading" ? "opacity-50 cursor-not-allowed" : ""}`}
          >
            {status === "loading" ? "Logging in..." : "Login"}
          </button>

          <span className="text-center text-lg text-gray-500">
            Don't have an account? <Link to="/register" className="font-semibold mx-2 text-blue-500 underline hover:text-blue-700">Register</Link>
          </span>
        </form>
      </div>
    </div>
